			...
			(cond ((and (string= (string (first solicitud)) "SELECCIÓN") (numberp (fourth solicitud))) 
				...
				((or (string= (string (first solicitud)) "SELECCIÓN") (string= (string (first solicitud)) "REVISIÓN"))
					...
					(let ((aexp1 (first (obtiene-expresion (list (parse-integer (second solicitud))))))
						...
						(recorre-Registros (mayor (parse-integer (third solicitud)) (parse-integer (second solicitud))) *mensajes-agente*)
						(recorre-Registros (menor (parse-integer (third solicitud)) (parse-integer (second solicitud))) *mensajes-agente*)) 
						(setf *cont* (- *cont* 2))
						(setf *cont4* (- *cont4* 2))
