...
;;  
;;=======================================================================================
(defun rules-N2 (rule noExp1 noExp2 &optional decimales)
	(labels (
		(comparasion (vv1 vv2)
		(let ((w+ 'nil) (w 'nil) ) 
	 	 (setq w+ (ext-and (first vv2) (second vv2) (first vv1) (second vv1)) 		;w+ = and(f2 , c2 , f1 , c1 )
...
(defun mayor (n1 n2)
	(let ((men 0))
		(cond ((> n1 n2)
			...
			(T (setf men n2)))
			men))
(defun eliminar (exp1 exp2)
	(let ((aexp1 (first (obtiene-expresion (list exp1))))
		(aexp2 (first (obtiene-expresion (list exp2)))))
		(elimina-Registros aexp2 *my-cache*)
		(elimina-Registros aexp1 *my-cache*)
		(recorre-Registros (mayor exp1 exp2) *my-cache*)
		(recorre-Registros (menor exp1 exp2) *my-cache*)
		(elimina-Registros aexp2 *mensajes-agente*)
		(elimina-Registros aexp1 *mensajes-agente*)
		(recorre-Registros (mayor exp1 exp2) *mensajes-agente*)
		(recorre-Registros (menor exp1 exp2) *mensajes-agente*)) 
		(setf *cont* (- *cont* 2))
		(setf *cont4* (- *cont4* 2)))
		 ...
		 ((and (< noExp1 *cont*) (< noExp2 *cont*))
			...
			(cond ((and (string= (string (first solicitud)) "SELECCIÓN") (numberp (fourth solicitud))) 
				...
				((or (string= (string (first solicitud)) "SELECCIÓN") (string= (string (first solicitud)) "REVISIÓN"))
					...
					(eliminar (parse-integer (second solicitud)) (parse-integer (third solicitud)))
				...
				((and (not (numberp (fourth solicitud)) ) (or (string= (string (first solicitud)) "COMPARACIÓN") (string= (string (first solicitud)) "ANALOGÍA") (string= (string (first solicitud)) "SEMEJANZA")))
					(rules-N2 (first solicitud) noExp1 noExp2 decimales) )
